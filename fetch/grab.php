...
$query4 = "SELECT schedule FROM settings where itemName = ?";

$stmt1 = $connection->stmt_init();

if ($stmt1->prepare($query4)) {

	$stmt1->bind_param("s",$item);
	
	$stmt1->execute();
	$stmt1->store_result();
	$stmt1->bind_result($sched);
	...
	$temp1 = "CREATE TEMPORARY TABLE IF NOT EXISTS switchOn AS (SELECT * From Item".$itemId." WHERE Value > 8)";
